                ...
                else
                {
                    ...
                    if ( (role_cvnho_cnt > 0  || role_cvnho_ass_cnt > 0)&& role_cvnch_cnt ==0)//NHO
                    {
                        ...
                        else
                        {
                            ...
                            " left outer join oversite_status_master s on s.ovrstatusid = t1.verification_status WHERE ( (t1.verification_status NOT IN (5, 4) AND NVL(t1.nho_verify, 0) = 0) OR  (t1.verification_status = 5 AND t1.NHO_sendback IN (2)) OR (t1.verification_status = 13 AND ops.roh_sendback IN (2)) OR (ops.roh_verify_by IS NOT NULL AND ops.roh_verify_date IS NOT NULL) )/*where  (t1.verification_status not in (5,4) and nvl(t1.nho_verify,0)=0 or (t1.verification_status=5  and t1.NHO_sendback in(2))or (t1.verification_status = 13 and ops.roh_sendback in(2))) */" + Qblock + "");
