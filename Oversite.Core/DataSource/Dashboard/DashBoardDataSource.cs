        ...
        public Response<DashBoardResponse> Data(DashBoardRequest request)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(request.status))
                {
                    ...
                    {
                        if (request.roleId == "78" || request.roleId == "77" || request.roleId == "79"  )
                        {
                ...
                else
                {
                    ...
                    decimal role_cvnho_cnt = new OracleHelper().ExecuteScalar<decimal>("select count(*) from lms_tw.vw_cv_User_Role_Details t1 left outer join lms_tw.vw_cv_user_master t2 ON  t1.user_id = t2.user_id  WHERE t2.emp_code = '" + request.enterBy + "' and t1.Role_id=81");//cv nho //cv oversite nho
                    decimal role_cvnch_cnt = new OracleHelper().ExecuteScalar<decimal>("select count(*) from lms_tw.vw_cv_User_Role_Details t1 left outer join lms_tw.vw_cv_user_master t2 ON  t1.user_id = t2.user_id  WHERE t2.emp_code = '" + request.enterBy + "' and t1.Role_id=77");//cv nch //cv oversite nch
                    ...
                    decimal role_cvrch_cnt = new OracleHelper().ExecuteScalar<decimal>("select count(*) from lms_tw.vw_cv_User_Role_Details t1 left outer join lms_tw.vw_cv_user_master t2 ON  t1.user_id = t2.user_id  WHERE t2.emp_code = '" + request.enterBy + "' and t1.Role_id=78");//cv rch //cv oversite rch
                    decimal role_cvroh_cnt = new OracleHelper().ExecuteScalar<decimal>("select count(*) from lms_tw.vw_cv_User_Role_Details t1 left outer join lms_tw.vw_cv_user_master t2 ON  t1.user_id = t2.user_id  WHERE t2.emp_code = '" + request.enterBy + "' and t1.Role_id=80");//cv rho //cv oversite rho

                    decimal role_cvbcm_cnt = new OracleHelper().ExecuteScalar<decimal>("select count(*) from lms_tw.vw_cv_User_Role_Details t1 left outer join lms_tw.vw_cv_user_master t2 ON  t1.user_id = t2.user_id  WHERE t2.emp_code = '" + request.enterBy + "' and t1.Role_id=79");//cv oversite bch //cv bcm
                                                                                                                                                                                                                                                                                          //100675
                    decimal role_bco = new OracleHelper().ExecuteScalar<decimal>("select count(*) from lms_tw.vw_cv_User_Role_Details t1 left outer join lms_tw.vw_cv_user_master t2 ON  t1.user_id = t2.user_id  WHERE t2.emp_code = '" + request.enterBy + "' and t1.Role_id=82");// cv oversite bho//cv bco

                    decimal role_report = new OracleHelper().ExecuteScalar<decimal>("select count(*) from lms_tw.vw_cv_User_Role_Details t1 left outer join lms_tw.vw_cv_user_master t2 ON  t1.user_id = t2.user_id  WHERE t2.emp_code = '" + request.enterBy + "' and t1.Role_id=83");// cv oversite bho//cv bco
                    ...
                    if (role_cvbcm_cnt > 0 && role_bco ==0)//BCM
                    {
                        ...
                        if (!string.IsNullOrEmpty(request.status.ToString()))
                        {
                            ...
                            {
                                Dashboardlist = new OracleHelper().GetRecords<DashBoardProperties>("select to_char(t1.Loan_Id) LoanId,to_char(t1.application_id) ApplicationNo,t1.customer_name cusName,to_char(t1.disbursed_date, 'dd-mm-yyyy') disDate,t2.branch_name branch,to_char(t1.loan_amount) as loanAmount,case nvl(t1.cr_verification_status, 0) when 2 then 'Verify By NCH' when 3 then case nvl(t1.nch_sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by NCH' end when 8 then 'Verify By RCH' when 9 then 'Verify By BCM' when 10 then case nvl(t1.rch_sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by RCH' end else 'Not Verified' end status,to_char(s.ovrstatusid) as statusId,'1' as redirectFlag,'79' roleid,c.num_weekdays TAT " +
                            ...
                            {
                                Dashboardlist = new OracleHelper().GetRecords<DashBoardProperties>("select to_char(t1.Loan_Id) LoanId,to_char(t1.application_id) ApplicationNo,t1.customer_name cusName,to_char(t1.disbursed_date, 'dd-mm-yyyy') disDate,t2.branch_name branch,to_char(t1.loan_amount) as loanAmount,case nvl(t1.cr_verification_status, 0) when 2 then 'Verify By NCH' when 3 then case nvl(t1.nch_sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by NCH' end when 8 then 'Verify By RCH' when 9 then 'Verify By BCM' when 10 then case nvl(t1.rch_sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by RCH' end else 'Not Verified' end status,to_char(s.ovrstatusid) as statusId,'0' as redirectFlag,'79' roleid,c.num_weekdays TAT " +
                    ...
                    if ( (role_cvnho_cnt > 0  || role_cvnho_ass_cnt > 0)&& role_cvnch_cnt ==0)//NHO
                    {
                        ...
                        {
                            Dashboardlist = new OracleHelper().GetRecords<DashBoardProperties>("select to_char(t1.Loan_Id) LoanId, to_char(t1.application_id) ApplicationNo, t1.customer_name cusName, to_char(t1.disbursed_date, 'dd-mm-yyyy') disDate, t2.branch_name branch, to_char(t1.loan_amount) as loanAmount, case nvl(t1.verification_status, 0) when 4 then 'Verify By NHO' when 5 then case nvl(t1.Nho_Sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by NHO' end when 11 then 'Verify By RHO' when 13 then 'Verify By BHO' when 12 then case nvl(ops.roh_sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by RHO' end else 'Not Verified' end status, '1' as statusId,case when ops.roh_verify is null then '0' else '1' end as redirectFlag,c.num_weekdays tat from TBL_OVERSITE_MASTER t1  left outer join tbl_ops_oversite_master ops on t1.application_id = ops.application_id and t1.loan_id = ops.loan_id CROSS APPLY ( SELECT COUNT (*) AS num_weekdays FROM dual WHERE TO_CHAR ( to_date(ops.roh_verify_date,'dd-mm-yyyy') + LEVEL , 'DY' , 'NLS_DATE_LANGUAGE=ENGLISH' ) NOT IN ('SUN') AND TO_CHAR ( to_date(ops.roh_verify_date,'dd-mm-yyyy') + LEVEL , 'DD-MM' , 'NLS_DATE_LANGUAGE=ENGLISH' ) NOT IN ('26-01','02-10','01-05','15-08') CONNECT BY LEVEL < sysdate - to_date(ops.roh_verify_date,'dd-mm-yyyy') ) c left outer join branch_master t2 on t1.branch_id = t2.branch_id " +
                    ...
                    if (role_cvnch_cnt > 0 && role_cvnho_cnt ==0) //NCH
                    {
                        ...
                        {
                            Dashboardlist = new OracleHelper().GetRecords<DashBoardProperties>("select to_char(t1.Loan_Id) LoanId,to_char(t1.application_id) ApplicationNo,t1.customer_name cusName,to_char(t1.disbursed_date,'dd-mm-yyyy') disDate,t2.branch_name branch,to_char(t1.loan_amount) as loanAmount,case nvl(t1.cr_verification_status, 0) when 2 then 'Verify By NCH' when 3 then case nvl(t1.nch_sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by NCH' end when 8 then 'Verify By RCH' when 9 then 'Verify By BCM' when 10 then case nvl(t1.rch_sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by RCH' end else 'Not Verified' end status,case t1.nch_sendback when 2 then '8' else to_char(s.ovrstatusid) end as statusId,case t1.nch_sendback when 1 then '0' else '1' end as redirectFlag,'77' roleid,c.num_weekdays TAT from TBL_OVERSITE_MASTER t1 CROSS APPLY ( SELECT COUNT (*) AS num_weekdays FROM dual WHERE TO_CHAR ( to_date(t1.disbursed_date,'dd-mm-yyyy') + LEVEL , 'DY' , 'NLS_DATE_LANGUAGE=ENGLISH' ) NOT IN ('SUN') AND TO_CHAR ( to_date(t1.disbursed_date,'dd-mm-yyyy') + LEVEL , 'DD-MM' , 'NLS_DATE_LANGUAGE=ENGLISH' ) NOT IN ('26-01','02-10','01-05','15-08') CONNECT BY LEVEL < sysdate - to_date(t1.disbursed_date,'dd-mm-yyyy') ) c left outer join branch_master t2 on t1.branch_id = t2.branch_id " +
                     ...
                     if (/*role_rch_cnt > 0 ||*/ role_cvrch_cnt > 0 && role_cvroh_cnt ==0)//RCH
                    {
                        ...
                        if (!string.IsNullOrEmpty(request.status.ToString()))
                        {
                            ...
                            {
                                Dashboardlist = new OracleHelper().GetRecords<DashBoardProperties>("select to_char(t1.Loan_Id) LoanId,to_char(t1.application_id) ApplicationNo,t1.customer_name cusName,to_char(t1.disbursed_date, 'dd-mm-yyyy') disDate,t2.branch_name branch,to_char(t1.loan_amount) as loanAmount,case nvl(t1.cr_verification_status, 0) when 2 then 'Verified By NCH' when 3 then case nvl(t1.nch_sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by NCH' end when 8 then 'Verified By RCH' when 9 then 'Verified By BCM' when 10 then case nvl(t1.rch_sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by RCH' end else 'Not Verified' end status,case t1.rch_sendback when 2 then case t1.nch_sendback when 1 then '3' else '9' end when 1 then to_char(s.ovrstatusid)else case t1.nch_sendback when 1 then '3' else to_char(s.ovrstatusid) end end  as statusId,case t1.rch_sendback when 1 then '0' else '1' end  as redirectFlag,'78' roleid,c.num_weekdays TAT from TBL_OVERSITE_MASTER t1 CROSS APPLY ( SELECT COUNT (*) AS num_weekdays FROM dual WHERE TO_CHAR ( to_date(t1.disbursed_date,'dd-mm-yyyy') + LEVEL , 'DY' , 'NLS_DATE_LANGUAGE=ENGLISH' ) NOT IN ('SUN') AND TO_CHAR ( to_date(t1.disbursed_date,'dd-mm-yyyy') + LEVEL , 'DD-MM' , 'NLS_DATE_LANGUAGE=ENGLISH' ) NOT IN ('26-01','02-10','01-05','15-08') CONNECT BY LEVEL < sysdate - to_date(t1.disbursed_date,'dd-mm-yyyy') ) c left outer join branch_master t2 on t1.branch_id = t2.branch_id left outer join oversite_status_master s on s.ovrstatusid = t1.cr_verification_status where (t1.verification_status not in (6) or   t1.application_id in(select distinct(tt.application_id) from TBL_OVERSITE_DOCUMENTS_MST tt where tt.nch_sendback_to='R') )" + Qblock + " order by c.num_weekdays desc");
                            }
                            else if (request.status == "3")

                            {
                                Dashboardlist = new OracleHelper().GetRecords<DashBoardProperties>("select to_char(t1.Loan_Id) LoanId,to_char(t1.application_id) ApplicationNo,t1.customer_name cusName,to_char(t1.disbursed_date, 'dd-mm-yyyy') disDate,t2.branch_name branch,to_char(t1.loan_amount) as loanAmount,case nvl(t1.cr_verification_status, 0) when 2 then 'Verified By NCH' when 3 then case nvl(t1.nch_sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by NCH' end when 8 then 'Verified By RCH' when 9 then 'Verified By BCM' when 10 then case nvl(t1.rch_sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by RCH' end else 'Not Verified' end status,case t1.rch_sendback when 2 then case t1.nch_sendback when 1 then '3' else '9' end when 1 then to_char(s.ovrstatusid)else case t1.nch_sendback when 1 then '3'  when 2 then '1' else to_char(s.ovrstatusid) end end  as statusId, CASE WHEN  t1.nch_sendback =1 THEN '0' ELSE '1' END as redirectFlag,'78' roleid,c.num_weekdays TAT  from TBL_OVERSITE_MASTER t1 CROSS APPLY ( SELECT COUNT (*) AS num_weekdays FROM dual WHERE TO_CHAR ( to_date(t1.disbursed_date,'dd-mm-yyyy') + LEVEL , 'DY' , 'NLS_DATE_LANGUAGE=ENGLISH' ) NOT IN ('SUN') AND TO_CHAR ( to_date(t1.disbursed_date,'dd-mm-yyyy') + LEVEL , 'DD-MM' , 'NLS_DATE_LANGUAGE=ENGLISH' ) NOT IN ('26-01','02-10','01-05','15-08') CONNECT BY LEVEL < sysdate - to_date(t1.disbursed_date,'dd-mm-yyyy') ) c left outer join branch_master t2 on t1.branch_id = t2.branch_id left outer join oversite_status_master s on s.ovrstatusid = t1.cr_verification_status  where ((t1.verification_status not in (6)) and(t1.nho_sendback = 1 or t1.cr_verification_status is not null or t1.application_id in(select distinct(tt.application_id) from TBL_OVERSITE_DOCUMENTS_MST tt where tt.nch_sendback_to='R'))) " + Qblock + " order by c.num_weekdays desc ");

                            }
                            else
                            {
                                Dashboardlist = new OracleHelper().GetRecords<DashBoardProperties>("select to_char(t1.Loan_Id) LoanId,to_char(t1.application_id) ApplicationNo,t1.customer_name cusName,to_char(t1.disbursed_date, 'dd-mm-yyyy') disDate,t2.branch_name branch,to_char(t1.loan_amount) as loanAmount,case nvl(t1.cr_verification_status, 0) when 2 then 'Verified By NCH' when 3 then case nvl(t1.nch_sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by NCH' end when 8 then 'Verified By RCH' when 9 then 'Verified By BCM' when 10 then case nvl(t1.rch_sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by RCH' end else 'Not Verified' end status,case t1.rch_sendback when 2 then case t1.nch_sendback when 1 then '3' else '9' end when 1 then to_char(s.ovrstatusid)else case t1.nch_sendback when 1 then '3' else to_char(s.ovrstatusid) end end  as statusId,'1' as redirectFlag,'78' roleid,c.num_weekdays TAT  from TBL_OVERSITE_MASTER t1 CROSS APPLY ( SELECT COUNT (*) AS num_weekdays FROM dual WHERE TO_CHAR ( to_date(t1.disbursed_date,'dd-mm-yyyy') + LEVEL , 'DY' , 'NLS_DATE_LANGUAGE=ENGLISH' ) NOT IN ('SUN') AND TO_CHAR ( to_date(t1.disbursed_date,'dd-mm-yyyy') + LEVEL , 'DD-MM' , 'NLS_DATE_LANGUAGE=ENGLISH' ) NOT IN ('26-01','02-10','01-05','15-08') CONNECT BY LEVEL < sysdate - to_date(t1.disbursed_date,'dd-mm-yyyy') ) c left outer join branch_master t2 on t1.branch_id = t2.branch_id left outer join oversite_status_master s on s.ovrstatusid = t1.cr_verification_status  where ((t1.verification_status not in (6)) and(t1.nho_sendback = 1 or t1.cr_verification_status is not null or t1.application_id in(select distinct(tt.application_id) from TBL_OVERSITE_DOCUMENTS_MST tt where tt.nch_sendback_to='R'))) " + Qblock + " order by c.num_weekdays desc ");
                        ...
                        else
                        {
                            Dashboardlist = new OracleHelper().GetRecords<DashBoardProperties>("select to_char(t1.Loan_Id) LoanId,to_char(t1.application_id) ApplicationNo,t1.customer_name cusName,to_char(t1.disbursed_date, 'dd-mm-yyyy') disDate,t2.branch_name branch,to_char(t1.loan_amount) as loanAmount,case nvl(t1.cr_verification_status, 0) when 2 then 'Verified By NCH' when 3 then case nvl(t1.nch_sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by NCH' end when 8 then 'Verified By RCH' when 9 then 'Verified By BCM' when 10 then case nvl(t1.rch_sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by RCH' end else 'Not Verified' end status,case t1.rch_sendback when 2 then case t1.nch_sendback when 1 then '3' else '9' end when 1 then to_char(s.ovrstatusid)else case t1.nch_sendback when 1 then '3' else to_char(s.ovrstatusid) end end  as statusId,case t1.rch_sendback when 1 then '0' else '1' end as redirectFlag,'78' roleid,c.num_weekdays TAT  from TBL_OVERSITE_MASTER t1 CROSS APPLY ( SELECT COUNT (*) AS num_weekdays FROM dual WHERE TO_CHAR ( to_date(t1.disbursed_date,'dd-mm-yyyy') + LEVEL , 'DY' , 'NLS_DATE_LANGUAGE=ENGLISH' ) NOT IN ('SUN') AND TO_CHAR ( to_date(t1.disbursed_date,'dd-mm-yyyy') + LEVEL , 'DD-MM' , 'NLS_DATE_LANGUAGE=ENGLISH' ) NOT IN ('26-01','02-10','01-05','15-08') CONNECT BY LEVEL < sysdate - to_date(t1.disbursed_date,'dd-mm-yyyy') ) c left outer join branch_master t2 on t1.branch_id = t2.branch_id left outer join oversite_status_master s on s.ovrstatusid = t1.cr_verification_status where ((t1.verification_status not in (6)) and( t1.nho_sendback = 1 or t1.rch_sendback=2 or t1.cr_verification_status not in(3,8) )) " + Qblock + "" +
                                "union select to_char(t1.Loan_Id) LoanId, to_char(t1.application_id) ApplicationNo, t1.customer_name cusName, to_char(t1.disbursed_date, 'dd-mm-yyyy') disDate, t2.branch_name branch, to_char(t1.loan_amount) as loanAmount, case nvl(t1.cr_verification_status, 0) when 2 then 'Verified By NCH' when 3 then case nvl(t1.nch_sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by NCH' end when 8 then 'Verified By RCH' when 9 then 'Verified By BCM' when 10 then case nvl(t1.rch_sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by RCH' end else 'Not Verified' end status, case t1.rch_sendback when 2 then case t1.nch_sendback when 1 then '3' else '9' end when 1 then to_char(s.ovrstatusid) else case t1.nch_sendback when 1 then '3' when 2 then '1' else to_char(s.ovrstatusid) end end as statusId, case t1.rch_sendback when 1 then '0' else '1' end as redirectFlag, '78' roleid, c.num_weekdays TAT from TBL_OVERSITE_MASTER t1 CROSS APPLY ( SELECT COUNT (*) AS num_weekdays FROM dual WHERE TO_CHAR ( to_date(t1.disbursed_date,'dd-mm-yyyy') + LEVEL , 'DY' , 'NLS_DATE_LANGUAGE=ENGLISH' ) NOT IN ('SUN') AND TO_CHAR ( to_date(t1.disbursed_date,'dd-mm-yyyy') + LEVEL , 'DD-MM' , 'NLS_DATE_LANGUAGE=ENGLISH' ) NOT IN ('26-01','02-10','01-05','15-08') CONNECT BY LEVEL < sysdate - to_date(t1.disbursed_date,'dd-mm-yyyy') ) c left outer join branch_master t2 on t1.branch_id = t2.branch_id left outer join oversite_status_master s on s.ovrstatusid = t1.cr_verification_status where t1.verification_status not in (6) and( t1.nch_sendback IS NULL OR t1.nch_sendback =2) and t1.rch_verify is null and t1.application_id in(select distinct(tt.application_id) from TBL_OVERSITE_DOCUMENTS_MST tt where tt.nch_sendback_to='R' " + Qblock + ") " +
                                "order by tat desc");
                    ...
                    else if (role_cvroh_cnt > 0 && role_cvrch_cnt ==0)//ROH
                    {
                        ...
                        if (!string.IsNullOrEmpty(request.status.ToString()))
                        {
                            ...
                            {
                                Dashboardlist = new OracleHelper().GetRecords<DashBoardProperties>("select to_char(t1.Loan_Id) LoanId, to_char(t1.application_id) ApplicationNo, t1.customer_name cusName, to_char(t1.disbursed_date, 'dd-mm-yyyy') disDate, t2.branch_name branch, to_char(t1.loan_amount) as loanAmount,  case nvl(t1.verification_status, 0) when 4 then 'Verified By NOH' when 5 then case nvl(t1.nho_Sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by NOH' end when 11 then 'Verified By ROH' when 13 then 'Verified By BCO' when 10 then case nvl(ops.roh_sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by ROH' end else 'Not Verified' end status, to_char(s.ovrstatusid) as statusId, case ops.roh_sendback when 2 then case t1.nho_sendback when 1 then '3' else '9' end when 1 then to_char(s.ovrstatusid) else case t1.nho_sendback when 1 then '3' else to_char(s.ovrstatusid) end end as statusId, '0' as redirectFlag, case ops.roh_sendback when 1 then '0' else '1' end as redirectFlag, '80' roleid, c.num_weekdays tat from TBL_OVERSITE_MASTER t1 CROSS APPLY (SELECT COUNT(*) AS num_weekdays FROM dual WHERE TO_CHAR(to_date(t1.disbursed_date, 'dd-mm-yyyy') + LEVEL, 'DY', 'NLS_DATE_LANGUAGE=ENGLISH') NOT IN ('SUN') AND TO_CHAR(to_date(t1.disbursed_date, 'dd-mm-yyyy') + LEVEL, 'DD-MM', 'NLS_DATE_LANGUAGE=ENGLISH') NOT IN ('26-01', '02-10', '01-05', '15-08') CONNECT BY LEVEL < sysdate - to_date(t1.disbursed_date, 'dd-mm-yyyy')) c " +
                            ...
                            else
                            {
                                ...
                                Dashboardlist = new OracleHelper().GetRecords<DashBoardProperties>("select to_char(t1.Loan_Id) LoanId, to_char(t1.application_id) ApplicationNo, t1.customer_name cusName, to_char(t1.disbursed_date, 'dd-mm-yyyy') disDate, t2.branch_name branch, to_char(t1.loan_amount) as loanAmount, /*--s.ovrdescription || decode(t1.NCH_sendback, 2, ' RECAPTURED') as status,*/ case nvl(t1.verification_status, 0) when 4 then 'Verified By NOH' when 5 then case nvl(t1.nho_Sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by NOH' end when 11 then 'Verified By ROH' when 13 then 'Verified By BCO' when 10 then case nvl(ops.roh_sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by ROH' end else 'Not Verified' end status, /*--to_char(s.ovrstatusid) as statusId*/ case ops.roh_sendback when 2 then case t1.nho_sendback when 1 then '3' else '9' end when 1 then to_char(s.ovrstatusid) else case t1.nho_sendback when 1 then '3' else to_char(s.ovrstatusid) end end as statusId, '1' as redirectFlag, '80' roleid, c.num_weekdays tat from TBL_OVERSITE_MASTER t1 CROSS APPLY (SELECT COUNT(*) AS num_weekdays FROM dual WHERE TO_CHAR(to_date(t1.disbursed_date, 'dd-mm-yyyy') + LEVEL, 'DY', 'NLS_DATE_LANGUAGE=ENGLISH') NOT IN ('SUN') AND TO_CHAR(to_date(t1.disbursed_date, 'dd-mm-yyyy') + LEVEL, 'DD-MM', 'NLS_DATE_LANGUAGE=ENGLISH') NOT IN ('26-01', '02-10', '01-05', '15-08') CONNECT BY LEVEL < sysdate - to_date(t1.disbursed_date, 'dd-mm-yyyy')) c " +
                        ...
                        else
                        {
                            ...
                            Dashboardlist = new OracleHelper().GetRecords<DashBoardProperties>("select to_char(t1.Loan_Id) LoanId, to_char(t1.application_id) ApplicationNo, t1.customer_name cusName, to_char(t1.disbursed_date, 'dd-mm-yyyy') disDate, t2.branch_name branch, to_char(t1.loan_amount) as loanAmount, case nvl(t1.verification_status, 0) when 4 then 'Verified By National Ops' when 5 then case nvl(t1.nho_sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by National Ops' end when 11 then 'Verified By Regianol Ops' when 13 then 'Verified By Branch Ops' when 12 then case nvl(ops.roh_sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by Regianol Ops' end else case nvl(ops.roh_sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by Regianol Ops' else 'Not Verified' end end status, case ops.roh_sendback when 2 then case t1.nho_sendback when 1 then '5' else '13' end when 1 then to_char(s.ovrstatusid) else case t1.nho_sendback when 1 then '5' else to_char(s.ovrstatusid) end end as statusId, case ops.roh_sendback when 1 then '0' else '1' end as redirectFlag, '80' roleid, c.num_weekdays tat from TBL_OVERSITE_MASTER t1 CROSS APPLY (SELECT COUNT(*) AS num_weekdays FROM dual WHERE TO_CHAR(to_date(t1.disbursed_date, 'dd-mm-yyyy') + LEVEL, 'DY', 'NLS_DATE_LANGUAGE=ENGLISH') NOT IN ('SUN') AND TO_CHAR(to_date(t1.disbursed_date, 'dd-mm-yyyy') + LEVEL, 'DD-MM', 'NLS_DATE_LANGUAGE=ENGLISH') NOT IN ('26-01', '02-10', '01-05', '15-08') CONNECT BY LEVEL < sysdate - to_date(t1.disbursed_date, 'dd-mm-yyyy')) c left outer join TBL_OPS_OVERSITE_MASTER ops on t1.application_id=ops.application_id left outer join branch_master t2 on t1.branch_id = t2.branch_id left outer join oversite_status_master s on s.ovrstatusid = t1.verification_status where (t1.verification_status not in (6) and t1.nho_sendback = 1) " + Qblock + " " +
                                   "union select to_char(t1.Loan_Id) LoanId, to_char(t1.application_id) ApplicationNo, t1.customer_name cusName, to_char(t1.disbursed_date, 'dd-mm-yyyy') disDate, t2.branch_name branch, to_char(t1.loan_amount) as loanAmount, case nvl(t1.verification_status, 0) when 4 then 'Verified By National Ops' when 5 then case nvl(t1.nho_sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by National Ops' end when 11 then 'Verified By Regianol Ops' when 13 then 'Verified By Branch Ops' when 12 then case nvl(ops.roh_sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by Regianol Ops' end else case nvl(ops.roh_sendback, 0) when 2 then 'Recaptured' when 1 then 'Sendback by Regianol Ops' else 'Not Verified' end end status, case ops.roh_sendback when 2 then case t1.nho_sendback when 1 then '5' else '13' end when 1 then to_char(s.ovrstatusid) else case t1.nho_sendback when 1 then '5' when 2 then '1' else to_char(s.ovrstatusid) end end as statusId,'1'  as redirectFlag, '80' roleid, c.num_weekdays tat from TBL_OVERSITE_MASTER t1 CROSS APPLY (SELECT COUNT(*) AS num_weekdays FROM dual WHERE TO_CHAR(to_date(t1.disbursed_date, 'dd-mm-yyyy') + LEVEL, 'DY', 'NLS_DATE_LANGUAGE=ENGLISH') NOT IN ('SUN') AND TO_CHAR(to_date(t1.disbursed_date, 'dd-mm-yyyy') + LEVEL, 'DD-MM', 'NLS_DATE_LANGUAGE=ENGLISH') NOT IN ('26-01', '02-10', '01-05', '15-08') CONNECT BY LEVEL < sysdate - to_date(t1.disbursed_date, 'dd-mm-yyyy')) c left outer join TBL_OPS_OVERSITE_MASTER ops on t1.application_id=ops.application_id left outer join branch_master t2 on t1.branch_id = t2.branch_id left outer join oversite_status_master s on s.ovrstatusid = t1.verification_status where (t1.verification_status not in (6) and (t1.nho_sendback is not null or ops.roh_verify is null or  ops.roh_sendback=2)) " + Qblock + " order by TAT desc");
